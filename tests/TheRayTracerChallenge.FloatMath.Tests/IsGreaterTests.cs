...
{
    public class IsGreaterTests
    {
        // Basically, if one float is greater and they're not equal, and none are NaNs, it's greater

        private const float TestingThreshold = 0.00001f; // Used for most tests, except like 2 or sth


        [Theory(DisplayName = nameof(Given_TwoFloats_Then_CorrectOrdering))]
        // Large Floats
        ...
        public void Given_TwoFloats_Then_CorrectOrdering(float a, float b, bool expected, float threshold = TestingThreshold)
        {
            bool result = Comparisons.IsGreater(a, b, threshold);
